...
class BenzaTrackerApp(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc | None = None, store: DataStore | None = None) -> None:
        ...
        self._selected_photo_path: Path | None = None
        self._photo_preview_window: tk.Toplevel | None = None
        self._photo_preview_image: ImageTk.PhotoImage | None = None
        ...
        # Build the full dashboard layout through a single helper.  This keeps
        # backwards compatibility with older versions of the application where
        # ``_build_layout`` was invoked from ``__init__`` and avoids the stale
        # ScrolledFrame usage that triggered AttributeError during PyInstaller
        # builds on Windows environments.
        self._build_layout()
    ...
    def _build_form(self) -> None:
        ...
        self.photo_label_var = tk.StringVar(value="Nessuna foto selezionata")
        ...
        photo_frame.grid(row=2, column=0, columnspan=len(fields) + 1, sticky="ew", padx=5, pady=(0, 10))
        photo_frame.columnconfigure(1, weight=1)

        ttk.Button(photo_frame, text="Scegli foto", command=self._on_select_photo).grid(
            row=0, column=0, sticky="w"
        )
        ttk.Label(photo_frame, textvariable=self.photo_label_var).grid(
            row=0, column=1, sticky="w", padx=10
        )
        ttk.Button(photo_frame, text="Rimuovi foto", command=self._clear_photo_selection).grid(
    ...
    def _build_table(self) -> None:
        ...
        columns = ("date", "liters", "amount", "price", "station", "odometer", "photo")
        ...
        for column, width in zip(columns, (120, 80, 100, 80, 160, 150, 80)):
        ...
        ttk.Button(button_frame, text="Apri foto", command=self._on_show_photo).grid(
    ...
    def _on_add_entry(self) -> None:
        ...
        if self._selected_photo_path is not None:
            try:
                photo_identifier = self.store.import_photo(self._selected_photo_path)
            except FileNotFoundError:
                messagebox.showerror(
                    "Foto non trovata",
                    "Il file selezionato non è più disponibile.",
                )
                return
            except OSError as exc:
                messagebox.showerror(
                    "Errore foto",
                    f"Impossibile salvare la foto selezionata: {exc}",
                )
                return
    ...
    def _refresh_table(self) -> None:
        ...
        for index, entry in enumerate(self._entries_cache):
            ...
            photo = "Sì" if entry.photo_path else "-"
            self.tree.insert(
                "",
                "end",
                iid=str(index),
    ...
    def _on_select_photo(self) -> None:
        filetypes = [
            ("File immagine", "*.png *.jpg *.jpeg *.bmp *.gif"),
            ("Tutti i file", "*.*"),
        ]
        ...
        if selected:
            self._selected_photo_path = Path(selected)
            self.photo_label_var.set(self._selected_photo_path.name)

    def _clear_photo_selection(self) -> None:
        self._selected_photo_path = None
        self.photo_label_var.set("Nessuna foto selezionata")

    def _on_show_photo(self) -> None:
        selection = self.tree.selection()
        if not selection:
            messagebox.showinfo("Nessuna selezione", "Seleziona un rifornimento per vedere la foto.")
            return

        index = int(selection[0])
        entry = self._entries_cache[index]
        if not entry.photo_path:
            messagebox.showinfo("Nessuna foto", "Il rifornimento selezionato non ha una foto associata.")
            return

        photo_file = self.store.resolve_photo_path(entry.photo_path)
        if not photo_file.exists():
            messagebox.showerror(
                "Foto non trovata",
                "Il file collegato non è più presente sul disco.",
            )
            return
        ...
        try:
            with Image.open(photo_file) as image:
                ...
                rendered = image.copy()
            photo_image = ImageTk.PhotoImage(rendered)
        except Exception as exc:  # pragma: no cover - dipende da file esterni
            messagebox.showerror(
                "Errore apertura foto",
                f"Impossibile aprire la foto selezionata: {exc}",
            )
            return

        if self._photo_preview_window is not None and self._photo_preview_window.winfo_exists():
            self._photo_preview_window.destroy()

        window = tk.Toplevel(self.root)
        window.title("Foto rifornimento")
        window.geometry("720x520")

        label = ttk.Label(window, image=photo_image)
        label.image = photo_image  # Conserva il riferimento
        label.pack(padx=10, pady=10)

        ttk.Label(window, text=str(photo_file)).pack(padx=10, pady=(0, 12))

        self._photo_preview_window = window
        self._photo_preview_image = photo_image
